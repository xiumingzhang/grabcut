...
public:
	...
	// Returns true if the residual capacity actually changed (useful for determining if a node must be 'marked' when re-using search trees)
	// NOTE: the number of such edges is not counted in edge_num_max.
	//       No internal memory is allocated by this call.
	bool add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline bool Graph<captype,tcaptype,flowtype>::add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
	assert(i >= 0 && i < node_num);
	tcaptype oldcap = nodes[i].tr_cap;
	...
	return !((nodes[i].tr_cap > 0 && oldcap > 0)||(nodes[i].tr_cap < 0 && oldcap < 0));
}
